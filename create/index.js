...
const inpForceMultiplyer = document.querySelector("#inp-force-multiplyer");
...
function setControlsForCelestialSimula(disabled) {
	...
	inpForceMultiplyer.disabled = !disabled;
}
...
function editParams() {
	...
	inpForceMultiplyer.value = forceMultiplyer || 10000;
...
function changeParams() {
	...
	initialState.forceMultiplyer = +inpForceMultiplyer.value || 10000;
...
function changePeriodicExtForce() {
	...
	const periodicExtForce = {
		...
		freqMax: +inpFreqMax.value || 0,
		omega: Math.PI * (+inpFreqMin.value + +inpFreqMax.value) || 0,
		F0x: +inpF0x.value || 0,
		F0y: +inpF0y.value || 0,
		fix: +inpFix.value || 0,
		fiy: +inpFiy.value || 0,
...
inpForceMultiplyer.addEventListener("change", () => changeParams());
