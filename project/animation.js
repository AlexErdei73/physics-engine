...
function drawForces(state, ctx) {
	...
	for (let i = 0; i < points.length; i++) {
		if (simulationType === "celestial" && newtonGravForces) {
			for (let j = 0; j < i; j++) {
				const force = newtonGravForces.find(
					(force) =>
						(force.point1 === i && force.point2 === j) ||
						(force.point1 === j && force.point2 === i)
				);
				drawVector(
...
function addGraphPoint(state, i) {
	...
	if (option === "rods") {
		...
		const point = state.points[index];
		const { vx, vy, ax, ay } = point;
		if (field === "v") y = Math.sqrt(vx * vx + vy * vy);
		if (field === "a") y = Math.sqrt(ax * ax + ay * ay);
	}
	graphs[i].push({ x, y });
}
