...
export function init(initialState) {
	...
	const { points } = state;
	points.forEach((point) => (point.s = 0));
}
...
function calcNewtonGravity(pointInd1, pointInd2, isMidpoint = false) {
	...
	r12.len = Math.sqrt(r12.x * r12.x + r12.y * r12.y);
	const PI = Math.PI;
	const G = 4 * PI * PI;
	const Fgr = (G * m1 * m2) / (r12.len * r12.len);
	const Fgrx = (-Fgr * r12.x) / r12.len;
	const Fgry = (-Fgr * r12.y) / r12.len;
	if (isMidpoint) {
		point2.axmid += Fgrx / m2;
		point2.aymid += Fgry / m2;
		point1.axmid -= Fgrx / m1;
		point1.aymid -= Fgry / m1;
	} else {
		point2.ax += Fgrx / m2;
		point2.ay += Fgry / m2;
		point1.ax -= Fgrx / m1;
	...
	return { point1: pointInd1, point2: pointInd2, r12, Fgr, Fgrx, Fgry };
}
...
function stepEulerRichardson() {
	...
	for (let i = 0; i < points.length; i++) {
		...
		if (!point.isFixed) {
			...
			point.x += vxmid * dt;
			point.y += vymid * dt;
			point.s += Math.sqrt(vxmid * vxmid + vymid * vymid) * dt;
		}
	}
}
