...
function handlePointIndexChange(event, rod) {
  ...
  const index = collisions.findIndex((col) => col.pointIndex === pointIndex);
  if (index === -1) {
...
function initResRod(state, index) {
  ...
  inpResRodCollPointIndex.addEventListener("change", (event) =>
    handlePointIndexChange(event, rod)
  );
